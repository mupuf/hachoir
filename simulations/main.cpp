...
typedef uint64_t (*scenario_t)(uint64_t hoppingPeriod, uint64_t beaconningPeriod,
		       size_t beaconCount, float senseBW);

uint64_t scenario1(uint64_t hoppingPeriod, uint64_t beaconningPeriod,
		       size_t beaconCount, float senseBW)
{
...
void beaconCount_vs_time(scenario_t scenario) {
	...
	for (size_t bc = 1; bc <= 10; bc++) {
		uint64_t sum = 0, sum_sq = 0, count = 100000, min = -1, max = 0;
		for (size_t i = 0; i < count; i++) {
			uint64_t res = scenario(hp_us, bp_us, bc, 25e6);
			sum_sq += (res * res);
			sum += res;
			if (res < min)
				min = res;
...
void bandwidth_vs_time(scenario_t scenario) {
	...
	for (float bandwidth = 4e6; bandwidth <= 100e6; bandwidth+=2e6) {
		...
		for (size_t i = 0; i < count; i++) {
			uint64_t res = scenario(hp_us, bp_us, bc, bandwidth);
			sum_sq += (res * res);
			sum += res;
			if (res < min)
				min = res;
