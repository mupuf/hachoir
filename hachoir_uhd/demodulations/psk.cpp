...
#if 0
uint8_t PSK::likeliness(const burst_sc16_t * const burst)
{
	size_t last_crossing = 0;
	Constellation c;
...
uint8_t PSK::likeliness(const Burst &burst)
{
	...
	for (size_t i = 0; i < burst.subBursts[0].len - 1; i++) {
		float ph1 = arg(std::complex<float>(burst.samples[i].real(), burst.samples[i].imag()));
		float ph2 = arg(std::complex<float>(burst.samples[i + 1].real(), burst.samples[i + 1].imag()));

		float dph = ph2 - ph1;
		if (dph < -M_PI)
			dph = (ph2 + 2 * M_PI) - ph1;

		std::cout << ph1 << " " << ph2 << " " << dph << std::endl;
		if (dph > 1 && dph < 2)
			std::cout << "Weird!" << std::endl;

		fprintf(f, "%f\n", dph);
		diff_phase.push_back(dph);

		cSymb.addPoint(dph * 1000.0);
		phase_sum += dph;
	}
	...
	for (size_t i = 0; i < diff_phase.size() - 1; i++) {
		if ((diff_phase[i] > _threshold && diff_phase[i + 1] <= _threshold) ||
			...
			symbolPSK s = { diff_phase_avr / cnt, (float)cnt};
